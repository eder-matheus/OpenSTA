...
public:
  ...
  virtual LibertyLibrary *readLiberty(std::string_view filename,
                                      ...
                                      bool infer_latches);
  // Load a LibertyLibrary from an LDB (Liberty database) file
  // produced by writeLibertyCache. Skips the Liberty text parse.
  // Falls through the same default-library / units / scene wiring as
  // readLiberty so callers can treat the result as equivalent to a
  // freshly-parsed library.

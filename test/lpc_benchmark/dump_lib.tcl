# LPC benchmark -- structural query dump.
#
# Sourced from phase1_write.tcl and phase2_read.tcl. Walks every
# cell / port / timing arc set / arc / internal_power / leakage_power
# / sequential / statetable via Tcl introspection and emits a
...
# internal_power and leakage_power groups, port flag bits, sequentials,
# statetable). The Tcl probe layer surfaces those facts.
#
# Usage:
#   source dump_lib.tcl
#   ::lpc_bench::dump_libraries /tmp/dump.txt

namespace eval ::lpc_bench {

# Format a float with fixed precision so two STA processes stamping
# the same value produce identical text. Avoids platform-dependent
# default formatting.
...
proc dump_cell {fp cell} {
  ...
  foreach as [$cell timing_arc_sets] {
    ...
    # via a typemap; `$as role` and `$arc from_edge` print directly.
    set role [$as role]
    set when [$as when]
    set sdf  [$as sdf_cond]
  ...
  # Power groups, sequentials, statetable -- via the LPC test-suite
  # SWIG probes at the bottom of Liberty.i. The data is stored in
  # cell->internalPowers() etc. and must round-trip through the
  # cache, but isn't reached by either write_liberty (it skips the
  # textual emission) or the standard Tcl introspection API.
...
proc dump_libraries {out_path} {
  ...
  foreach pair [lsort -index 0 -dictionary $lib_names] {
    ...
    # Sort cells for determinism -- get_lib_cells iteration order
    # isn't documented as stable across load methods.
    set cell_names {}
